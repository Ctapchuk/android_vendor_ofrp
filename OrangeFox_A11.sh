...
#  fi
  ...
  if [ "$FOX_DELETE_MAGISK_ADDON" = "1" ]; then
     ...
     rm -rf $FOX_RAMDISK/FFiles/OF_Magisk
  fi
  ...
  if [ -n "$FOX_USE_SPECIFIC_MAGISK_ZIP" ]; then
     if [ -e $FOX_USE_SPECIFIC_MAGISK_ZIP ]; then
        ...
        $CP -pf $FOX_USE_SPECIFIC_MAGISK_ZIP $FOX_RAMDISK/FFiles/OF_Magisk/Magisk.zip
     else
...
#########################################################################################
  ...
  # Get Magisk version
  tmp1=$FOX_VENDOR_PATH/FoxExtras/FFiles/OF_Magisk/Magisk.zip
